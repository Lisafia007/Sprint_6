...
class BasePageScooter():
   ...
   def close_cookie(self):
      WebDriverWait(self.driver, 10).until(expected_conditions.visibility_of_element_located(BasePageLocators.COOKIE_FIELD))
      cookie_button = self.driver.find_element(*BasePageLocators.COOKIE_CLOSE_BUTTON)
      cookie_button.click()

   #Метод 
   @allure.step("Ожидаем загрузки заголовка формы 'Для кого самокат'" )
   def wait_for_data_form(self):
      WebDriverWait(self.driver, 5).until(expected_conditions.visibility_of_element_located(BasePageLocators.HEADER_DATA_FORM))

   #Метод введения данных в поле Имя
   @allure.step("Вводим данные в поле Имя")
   def input_name(self, data):
      self.driver.find_element(*BasePageLocators.NAME_FIELD).click()
      self.driver.find_element(*BasePageLocators.NAME_FIELD).send_keys(data["name_field"])

   #Метод введения данных в поле Фамилия 
   @allure.step("Вводим данные в поле Фамилия")
   def input_surname(self, data):
      self.driver.find_element(*BasePageLocators.SURNAME_FIELD).click()
      self.driver.find_element(*BasePageLocators.SURNAME_FIELD).send_keys(data["surname_field"])

   #Метод введения данных в поле Адрес
   @allure.step("Вводим данные в поле Адрес")
   def input_address(self, data):
      self.driver.find_element(*BasePageLocators.ADDRESS_FIELD).click()
      self.driver.find_element(*BasePageLocators.ADDRESS_FIELD).send_keys(data["address_field"])
   
   #Метод выбора станции метро
   @allure.step("Выбираем станцию метро из списка")
   def choose_metro(self, data):
      self.driver.find_element(*BasePageLocators.METRO_FIELD).click()
      metro = data['metro_name']
      WebDriverWait(self.driver, 5).until(expected_conditions.element_to_be_clickable((By.XPATH, f"//div[text()='{metro}']"))).click()

   #Метод введения данных в поле Телефон
   @allure.step("Вводим данные в поле Телефон")
   def input_phone(self, data):
      self.driver.find_element(*BasePageLocators.PHONE_FIELD).click()
      self.driver.find_element(*BasePageLocators.PHONE_FIELD).send_keys(data["phone_field"])
   ...
   def go_to_page(self):
      self.driver.find_element(*BasePageLocators.NEXT_BUTTON).click()
   ...
   def choose_day(self, data):
      self.driver.find_element(*BasePageLocators.DATA_DAY_FIELD).click()
      day = (data["data_day"])
      select_day = (By.XPATH, f"//div[contains(@class, 'react-datepicker__day') and text()='{day}']")
      WebDriverWait(self.driver, 5).until(expected_conditions.element_to_be_clickable(select_day)).click()

   #Метод выбора срока аренды
   @allure.step("Выбираем срок аренды")
   def choose_rental_time(self, data):
      self.driver.find_element(*BasePageLocators.RENTAL_TIME_FIELD).click()
      rental_days = data['rental_time']
      WebDriverWait(self.driver, 5).until(expected_conditions.element_to_be_clickable((By.XPATH, f"//div[text()='{rental_days}']"))).click()

   #Метод выбора цвета самоката
   @allure.step("Выбираем цвета самоката")
   def choose_color(self, data):
      self.driver.find_element(*BasePageLocators.CHOOSE_COLOR_SCOOTER).click()
      color_button = data['color']
      WebDriverWait(self.driver, 5).until(expected_conditions.element_to_be_clickable((By.ID, color_button))).click()
   ...
   def confirm_order(self):
      WebDriverWait(self.driver, 5).until(expected_conditions.visibility_of_element_located(BasePageLocators.CONFIRM_WINDOW))
      self.driver.find_element(*BasePageLocators.YES_BUTTON).click()

   #Метод появления окна с успешным заказом
   @allure.step("Появление окна с успешным заказом")
   def show_success_order(self):
      WebDriverWait(self.driver, 5).until(expected_conditions.visibility_of_element_located(BasePageLocators.ORDER_PLACED_WINDOW))

   #Метод перехода по кнопке Посмотреть статус
   @allure.step("Нажимаем на кнопку Посмотреть статус")
   def show_status(self):
      WebDriverWait(self.driver, 5).until(expected_conditions.element_to_be_clickable(BasePageLocators.STATUS_BUTTON))
      self.driver.find_element(*BasePageLocators.STATUS_BUTTON).click()
